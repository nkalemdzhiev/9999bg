defmodule Nine999BgWeb.TeamLive.Show do
  use Nine999BgWeb, :live_view

  alias Nine999Bg.TeamStatus
  ...
  def render(assigns) do
    ...
    <main class="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div :if={!@status} class="rounded-lg border border-dashed border-slate-300 p-8 text-center">
        ...
        <p class="mt-2 text-sm text-slate-600">The team status could not be loaded yet.</p>
      </div>
        ...
        <section>
          ...
          <div :if={!Enum.empty?(players(@status))} class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            <article :for={player <- players(@status)} class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <p class="mt-1 text-sm text-slate-600"><%= value(player, :position, "Position unavailable") %></p>
            </article>
          </div>
        </section>
          ...
          <div :if={!Enum.empty?(recent_matches(@status))} class="space-y-3">
            <article :for={match <- recent_matches(@status)} class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <span class="text-sm text-slate-500"><%= value(match, :kickoff_at, "TBD") %></span>
              </div>
              <p class="mt-1 text-sm text-slate-600"><%= value(match, :status, "Finished") %></p>
            </article>
          </div>
        </section>
        ...
        <section>
          ...
          <ul :if={!Enum.empty?(honors(@status))} class="grid gap-3 sm:grid-cols-2">
            <li :for={honor <- honors(@status)} class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              <span class="font-semibold text-slate-950"><%= honor_title(honor) %></span>
            </li>
          </ul>
        </section>
      </section>
  ...
  defp fixture_name(match) do
    home = match |> value(:home_team) |> team_name()
    away = match |> value(:away_team) |> team_name()
    "#{home} vs #{away}"
  end

  defp honor_title(honor) when is_binary(honor), do: honor
  defp honor_title(honor), do: value(honor, :title, value(honor, :name, "Honor"))
